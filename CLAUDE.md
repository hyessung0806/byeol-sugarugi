...
2. **자동 업데이트(v9~):** `sw.js`가 HTML을 **네트워크 우선**으로 받으므로 `index.html` 내용을 고쳐 push하면 태블릿에서 **앱을 (온라인으로) 다시 열 때 자동 반영**된다. 캐시 버전(`byeolsugarugi-vN`)은 더 이상 매번 올릴 필요 없고, **`sw.js` 자체나 `ASSETS` 프리캐시 목록을 바꿀 때만** 올린다.
